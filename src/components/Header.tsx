        ...
        <div className="flex items-center justify-between">
          ...
          <NavigationMenu className="hidden lg:flex">
            <NavigationMenuList>
                  ...
                  <ul className="grid gap-3 p-4 md:w-[400px] lg:w-[500px] lg:grid-cols-2">
                    <li className="row-span-3">
                      <NavigationMenuLink asChild>
                        <Link to="/batch-trace" className="flex h-full w-full select-none flex-col justify-end rounded-lg bg-gradient-hero p-6 no-underline outline-none focus:shadow-md">
                          <QrCode className="h-5 w-5 text-primary" />
                          <div className="mb-2 mt-4 text-lg font-semibold text-primary-foreground">Trace Batch</div>
                          <p className="text-sm leading-tight text-primary-foreground/60">Complete farm-to-retail journey tracking.</p>
                        </Link>
                      </NavigationMenuLink>
                    </li>
              ...
              <NavigationMenuItem>
                <Link to="/pricing" className="text-sm font-medium text-foreground hover:text-primary transition-colors px-3 py-2">Pricing</Link>
              </NavigationMenuItem>
              <NavigationMenuItem>
                <Link to="/contact" className="text-sm font-medium text-foreground hover:text-primary transition-colors px-3 py-2">Contact</Link>
              </NavigationMenuItem>
            </NavigationMenuList>
          </NavigationMenu>
          ...
          <div className="flex items-center space-x-2">
            {user ? (
              <>
                ...
                <Button variant="ghost" size="sm" onClick={() => navigate('/silir')}>Dashboard</Button>
                <Button variant="outline" size="sm" onClick={async () => { await signOut(); navigate('/'); }}>Sign Out</Button>
              </>
            ) : (
              <>
                <Button variant="ghost" size="sm" onClick={() => navigate('/know-your-fruit')}>
                  <QrCode className="w-4 h-4 mr-1" />Know Your Fruit
                </Button>
                <Button size="sm" className="bg-primary text-primary-foreground hover:bg-primary-glow" onClick={() => navigate('/auth')}>Sign In</Button>
              </>
            )}
          </div>
        </div>
...
const ListItem = forwardRef<React.ElementRef<"a">, ListItemProps>(
  ({ className, title, children, icon, href, ...props }, ref) => (
    <li>
      <NavigationMenuLink asChild>
        <Link to={href} ref={ref} className={cn("block select-none space-y-1 rounded-lg p-3 leading-none no-underline outline-none transition-colors hover:bg-accent hover:text-accent-foreground", className)} {...props}>
